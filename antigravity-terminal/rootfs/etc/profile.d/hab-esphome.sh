...
hab() {
    if [ "$1" = "esphome" ] && [ -z "$HA_ACCESS_TOKEN" ]; then
        ...
        echo "  3. Go to Settings -> Add-ons -> Antigravity CLI Terminal -> Configuration" >&2
        echo "  4. Paste the token into the 'access_token' field" >&2
        echo "  5. Restart the Antigravity CLI Terminal add-on (with ESPHome already running)" >&2
        return 1
    fi
    command hab "$@"
}
