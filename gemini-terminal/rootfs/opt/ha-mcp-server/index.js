...
const ESPHOME_TOKEN_ERROR = "ESPHome tools require a Long-Lived Access Token.\n\n" +
  ...
  "3. Go to Settings → Add-ons → Gemini Terminal → Configuration\n" +
  "4. Paste the token into the 'access_token' field\n" +
  "5. Restart the Gemini Terminal add-on (with ESPHome already running)";
...
const SCREENSHOT_DISABLED_ERROR = "Screenshot tool is disabled.\n\n" +
  ...
  "1. Go to Settings → Add-ons → Gemini Terminal → Configuration\n" +
  "2. Enable 'Screenshot tool'\n" +
  "3. Set a Long-Lived Access Token (Profile → Long-Lived Access Tokens)\n" +
  "4. Restart the Gemini Terminal add-on";
...
const SCREENSHOT_TOKEN_ERROR = "Screenshot tool requires a Long-Lived Access Token.\n\n" +
  ...
  "3. Go to Settings → Add-ons → Gemini Terminal → Configuration\n" +
  "4. Paste the token into the 'access_token' field\n" +
  "5. Restart the Gemini Terminal add-on";
...
const TOOLS = [
  ...
  },
  {
    ...
    description: "Initiate an update for a Home Assistant component (Core, OS, Supervisor) or an app. Returns a job_id for progress monitoring. NOTE: Cannot update Gemini Terminal itself from within - use Home Assistant UI for self-updates.",
    inputSchema: {
      type: "object",
      properties: {
        component: {
  ...
  try {
    ...
    switch (name) {
      ...
      case "update_component": {
        ...
        // Prevent self-update
        if (component === "addon" && addon_slug === "gemini_terminal") {
          throw new Error("Cannot update Gemini Terminal from within itself. The container will be stopped during update. Please use the Home Assistant UI to update this app.");
        }
