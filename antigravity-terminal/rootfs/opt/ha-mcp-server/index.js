#!/usr/bin/env node
/**
 * Home Assistant MCP Server (Safe Config Edition v2.7)
 * 
...
// Refactored modules (Fix #4 – extracted from index.js for testability and maintainability)
import { callHA, callSupervisor, discoverHACoreUrl } from "./lib/ha-api.js";
import { takeScreenshot, validateScreenshotUrl } from "./lib/screenshot.js";
...
const ESPHOME_TOKEN_ERROR = "ESPHome tools require a Long-Lived Access Token.\n\n" +
  ...
  "3. Go to Settings → Add-ons → Antigravity CLI Terminal → Configuration\n" +
  "4. Paste the token into the 'access_token' field\n" +
  "5. Restart the Antigravity CLI Terminal add-on (with ESPHome already running)";
...
const SCREENSHOT_DISABLED_ERROR = "Screenshot tool is disabled.\n\n" +
  ...
  "1. Go to Settings → Add-ons → Antigravity CLI Terminal → Configuration\n" +
  "2. Enable 'Screenshot tool'\n" +
  "3. Set a Long-Lived Access Token (Profile → Long-Lived Access Tokens)\n" +
  "4. Restart the Antigravity CLI Terminal add-on";
...
const SCREENSHOT_TOKEN_ERROR = "Screenshot tool requires a Long-Lived Access Token.\n\n" +
  ...
  "3. Go to Settings → Add-ons → Antigravity CLI Terminal → Configuration\n" +
  "4. Paste the token into the 'access_token' field\n" +
  "5. Restart the Antigravity CLI Terminal add-on";
...
// NOTE: callHA(), callSupervisor(), and discoverHACoreUrl() have been moved
// to ./lib/ha-api.js (Fix #4 – modularisation). They are imported above.
// NOTE: takeScreenshot() and validateScreenshotUrl() have been moved
// to ./lib/screenshot.js (Fix #2 + #4). They are imported above.
...
const TOOLS = [
  ...
  },
  {
    ...
    description: "Initiate an update for a Home Assistant component (Core, OS, Supervisor) or an app. Returns a job_id for progress monitoring. NOTE: Cannot update Antigravity CLI Terminal itself from within - use Home Assistant UI for self-updates.",
    inputSchema: {
      type: "object",
      properties: {
        component: {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  // Helper to strip unsupported MCP features from response for compatibility
  const makeCompatibleResponse = (result) => {
    // Keep only standard fields: content, isError
    // Remove: structuredContent, resourceLinks (not supported by some clients)
    return {
  ...
  try {
    ...
    switch (name) {
      ...
      case "update_component": {
        ...
        // Prevent self-update
        if (component === "addon" && (addon_slug === "antigravity_terminal" || addon_slug === "gemini_terminal")) {
          throw new Error("Cannot update Antigravity CLI Terminal from within itself. The container will be stopped during update. Please use the Home Assistant UI to update this app.");
        }
